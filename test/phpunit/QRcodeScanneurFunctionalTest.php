...
/**
 * \file    test/phpunit/QRcodeScanneurFunctionalTest.php
...
/**
 * Class QRcodeScanneurFunctionalTest
 *
 ...
 *
 * @package Testqrcodescanneur
 */
class QRcodeScanneurFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modQRcodeScanneur')->click();
		} else {
			// Disable the module
			$this->byHref('modQRcodeScanneur')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/qrcodescanneur/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('qrcodescanneur/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/qrcodescanneur/admin/about.php');
		$this->authenticate();
		return $this->assertContains('qrcodescanneur/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('qrcodescanneurwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modQRcodeScanneur_QRcodeScanneurTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modQRcodeScanneur_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
